...
#USING "THE" TO FIND KEY
# keys=[]
# for i in range(0,len(cipher),3):
#     if(cipher[i] ^ 116 in  range(97,123) and cipher[i + 1]^104 in  range(97,123) and cipher[i + 2]^101 in  range(97,123)):
#         key = chr(cipher[i] ^ 116) + chr(cipher[i + 1]^104) + chr(cipher[i + 2]^101)
#         keys.append(key)
# print(sorted(keys,key=keys.count,reverse=True))

sum=0
for i in range(0,len(cipher),3):
    print(chr(cipher[i] ^ 101) + chr(cipher[i + 1]^120) + chr(cipher[i + 2]^112), end='' )
    sum+= (cipher[i]^101)+(cipher[i+1]^120)+(cipher[i+2]^112)
print()
print(sum)